...
def attach_ml_to_capture(
    ...
    if mode == "tcpdump":
        ...
        analysis = create_live_session(config, interface=iface, capture_source="scapy")
    else:
        eve_path = _resolve_eve_path(config, eve_hint)
        if not eve_path:
            return {"session_id": None, "status": "no_eve", "mode": mode}
        analysis = create_live_session(
            config, eve_path=eve_path, interface=iface, capture_source="suricata"
        )

    kickoff_live_monitor_current(analysis.id)
    capture_orchestrator.set_ml_session_id(config, analysis.id)
    logger.info("ML live session attached: %s mode=%s iface=%s", analysis.id, mode, iface)
    return {"session_id": analysis.id, "status": "running", "mode": mode}
